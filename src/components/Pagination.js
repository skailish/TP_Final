...
const Pagination = () => {
  const { page, maxPage, toPreviousPage, toNextPage } = useContext(
    PaginationContext
  );
  ...
  return (
    <>
      <Container className={`pagination-container ${theme}`}>
        ...
        {(page < 5) &&
          <>
            {[...Array(5)].map((pageBtn, i) => <PageButton key={i + 1} value={i + 1} content={i + 1} />)}
        ...
        {(page >= maxPage - 4 &&
          <>
            ...
            <PageButton value={maxPage - 4} key={maxPage - 4} content={maxPage - 4} />
            <PageButton value={maxPage - 3} key={maxPage - 3} content={maxPage - 3} />
            <PageButton value={maxPage - 2} key={maxPage - 2} content={maxPage - 2} />
            <PageButton value={maxPage - 1} key={maxPage - 1} content={maxPage - 1} />
            <PageButton value={maxPage} key={maxPage} content={maxPage} />
          </>
        )}
        {page < maxPage && (
